...
func _ready() -> void:
	for kind in BlockData.kinds():
		...
		for block_name in BlockData.blocks()[kind]:
			var block := Block.new(kind, block_name)
			block.drag_started.connect(self._on_drag_started)
...
func _on_drag_started(node: Block, offset: Vector2):
	node._is_dragging = false
	self._is_dragging = true
	self._drag_offset = offset
	self._now_drag_node = Block.new(node._kind, node._block_name)
